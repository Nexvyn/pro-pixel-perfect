...
const Floating = ({
  ...
  useAnimationFrame(() => {
    // Skip animation if reduced motion is preferred or component is not visible
    if (!containerRef.current || shouldReduceMotion || !isVisible) return
...
export const FloatingElement = ({ children, className, depth = 1 }: FloatingElementProps) => {
  ...
  return (
    <div
      ref={elementRef}
      className={cn("absolute", !shouldReduceMotion && "will-change-transform", className)}
